...
<div class="row">
	<div class="col-sm-4">
		<form id="frmVentasProductos">
			...
			<select class="form-select" id="clienteVenta" name="clienteVenta">
			...
			<select class="form-select" id="productoVenta" name="productoVenta">
				<option value="A">Selecciona</option>
				<?php
			...
			<textarea readonly id="descripcionV" name="descripcionV" class="form-control"></textarea>
			<label for="cantidadV">Cantidad</label>
			<input readonly type="text" class="form-control" id="cantidadV" name="cantidadV">
			<label for="precioV">Precio</label>
			<input readonly type="text" class="form-control" id="precioV" name="precioV">
			<p></p>
			<button type="button" class="btn btn-primary" id="btnAgregaVenta">Agregar</button>
		...
		$.ajax({
			...
			success: function (r) {
				if (r > 0) {
					...
					alertify.alert("Venta creada, consultar información en ventas hechas");
				} else if (r == 0) {
					alertify.alert("No hay valores en la lista de venta!!");
				} else {
					alertify.error("No se pudo completar la venta");
				}
			}
		});
	}
